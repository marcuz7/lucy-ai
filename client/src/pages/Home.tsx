...
export default function Home() {
  ...
  return <div id="top" className="site-shell"><Header onText={() => setModal(true)} /><main>
    ...
    <section className="benefits"><div><Check /> Start from a message</div><div><Check /> No installation</div><div><Check /> Help that fits the moment</div></section>
    <section className="intro section-pad"><span className="eyebrow">BUILT FOR THE WAY YOU ALREADY TALK</span><h2>Bring an agent<br className="mobile-only" /> into the thread.</h2><p>Lucy is not another place to keep open. It is an agent you can call into the conversation when a plan needs making, a question needs answering, or an idea needs momentum.</p><p className="small-copy">Start one-on-one, then invite Lucy wherever the work or the group chat is happening.</p><div className="assistant-card"><div className="assistant-dot">✦</div><h3>Lucy</h3><span>YOUR AI AGENT</span><div className="assistant-buttons"><button>message</button><button>add to<br />group</button></div></div></section>
    <section className="uses section-pad"><div className="uses-heading"><span className="eyebrow">ONE MESSAGE. MANY MODES.</span><h2>Think it. Text it.<br />Move it forward.</h2><p>Lucy turns everyday conversations into useful progress without asking you to change your workflow.</p></div><div className="feature-list"><FeatureCard image={pickleballImage} eyebrow="MAKE A PLAN" title="Turn “what should we do?” into a real plan" body="Lucy gathers the constraints, finds the options, and brings the group to a decision everyone can act on." /><FeatureCard image={dinnerImage} eyebrow="MAKE A CALL" title="Get past the endless maybe" body="Give Lucy the preferences and the tradeoffs. It will narrow the choices and explain the recommendation clearly." /><FeatureCard eyebrow="MAKE IT CLEAR" title="Shape the message before you send it" body="Drop in the rough version. Lucy can make it warmer, sharper, shorter, or easier to understand."><div className="mini-chat"><span>make this sound confident, not cold</span><span>Try this: “I’m excited to move ahead. Here’s what I need next…”</span></div></FeatureCard><FeatureCard eyebrow="MAKE SOMETHING" title="Turn a spark into a creative draft" body="Describe the mood, image, song, or idea. Lucy gives the thought a first form you can build on."><div className="music-note">✦ first draft ready · want it bolder or more surprising?</div></FeatureCard><FeatureCard eyebrow="GET UNSTUCK" title="Ask the question you keep circling" body="Compare, summarize, decode, research, or simply ask for the next step. Lucy helps you start." /><FeatureCard image={cabinImage} eyebrow="KEEP IT MOVING" title="Remember the details that matter" body="Lucy can keep track of the people, dates, preferences, and decisions that make a plan hold together." /></div></section>
